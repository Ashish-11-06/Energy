...
const formatCurrency = (amount, asSymbol = true) => {
  ...
  // Remove any "1 - " prefix if present
  let value = String(amount).replace(/'/g, '').replace(/^1\s*-\s*/, '');
  
  return (asSymbol ? '₹' : '') + new Intl.NumberFormat('en-IN', {
...
export const handleDownloadPDF = (
  ...
  doc.text(
    `Report generated for: ${userData?.name || "N/A"}`,
    20,
    55
  );
  doc.text(
  ...
  doc.text(
    `Project ID: ${selectedRequirement?.id || "N/A"}`,
    190,
    55,
    { align: "right" }
  );
